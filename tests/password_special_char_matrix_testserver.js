...
async function run() {
  const baseUrl = 'https://accounts-test.easywebinar.com/signup?planId=All-In-One-USD-Yearly-200';
  const specials = [
    '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '_', '=', '+',
    '[', ']', '{', '}', '\\', '|', ';', ':', "'", '"', ',', '.', '<', '>', '/', '?', '`', '~'
  ];
  ...
  const outPath = path.join(outDir, 'Password-Special-Character-Matrix-Results-TestServer.json');
